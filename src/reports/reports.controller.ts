import { Serialize } from 'src/interceptors/serialize-interceptor';
import {
  Body,
  Controller,
  Post,
  UseGuards,
  Param,
  Patch,
  Get,
  Query,
} from '@nestjs/common';

import { CreateReportDto } from './dtos/create-report.dto';
import { ReportsService } from './reports.service';
import { AuthGurad } from 'src/gurads/auth.gurad';
import { CurrentUser } from 'src/users/decorators/current-user.decorator';
import { User } from 'src/users/user.entity';
import { ReportDto } from './dtos/report.dto';
import { AproveReportDto } from './dtos/approve-report.dto';
import { AdminGuard } from 'src/gurads/admin-guard';
import { GetEstimateDto } from './dtos/get-estimate.dto';

@Controller('reports')
export class ReportsController {
  constructor(private reportService: ReportsService) {}

  @Post()
  @UseGuards(AuthGurad)
  @Serialize(ReportDto)
  async createReport(@Body() body: CreateReportDto, @CurrentUser() user: User) {
    return await this.reportService.create(body, user);
  }

  @Patch('/:id')
  @UseGuards(AdminGuard)
  approveReport(@Param('id') id: string, @Body() body: AproveReportDto) {
    return this.reportService.changeAproval(id, body.approved);
  }

  @Get()
  async getEstimate(@Query() query: GetEstimateDto) {
    return await this.reportService.createEstimate(query);
  }
}
